...
PROJECT_NAME          ?= convex_hull
PROJECT_VERSION       ?= 1.0
PROJECT_BUILD_PATH    ?= .

ifeq ($(PLATFORM),PLATFORM_DESKTOP)
    RAYLIB_PATH           ?= ./external/raylib-desktop
endif

ifeq ($(PLATFORM),PLATFORM_WEB)
    RAYLIB_PATH           ?= ./external/raylib-web
endif
...
ifeq ($(PLATFORM),PLATFORM_WEB)
    ...
    LDLIBS = $(RAYLIB_RELEASE_PATH)/lib/libraylib.a
endif
...
%.o: %.c
	$(CC) -c $< -o $(OUTPUT_DIR)/$@ $(CFLAGS) $(INCLUDE_PATHS) -D$(PLATFORM)
...
ifeq ($(PLATFORM),PLATFORM_WEB)
	emrun --port 3000 $(OUTPUT_DIR)/$(PROJECT_NAME).html
endif
